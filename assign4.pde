...
void setup() {
  ...
  for (int l = 0; l < 8; l++) {
    for (int k = 8; k < 16; k ++) {
      if (int(l  % 4) == 0 || int(l % 4) == 3) {
        if (int(k  % 4) == 1 || int(k % 4) == 2) {
          soilHealth[l][k] = 30;
        }
      }
      if (int(l % 4) == 1 || int(l % 4) == 2) {
        if (int(k % 4) == 0 || int(k % 4) == 3) {
          soilHealth[l][k] = 30;
        }
      }
    }
...
for (int i = 1; i < 24; i++) {
  ...
  for (int j = 0; j<num; j++) {
    ...
    //println(ceil(random(SOIL_COL_COUNT)) + " " + i);
  }
}
